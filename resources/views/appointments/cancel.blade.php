...
<div class="card shadow">
            ...
            <div class="card-body">
                ...
                @endif
                  ...
                  @if($role == 'paciente')
                    <p>Se cancelara tú cita reservada con el medico <b>{{ $appointment->doctor->name}}</b> 
                        (especialidad <b>{{ $appointment->specialty->name }}</b>) 
                        para el día <b>{{ $appointment->scheduled_date }}</b>. </p>

                  @elseif($role == 'doctor') 
                    <p>Se cancelara tú cita médica del paciente <b>{{ $appointment->patient->name}}</b> 
                          (especialidad <b>{{ $appointment->specialty->name }}</b>) 
                ...
                <form action=" {{ url('/miscitas/'.$appointment->id.'/cancel') }}" method="POST">
                    ...
                    <div class="form-group">
                        <label for="justification">Escriba los motivos de la cancelacion:</label>
